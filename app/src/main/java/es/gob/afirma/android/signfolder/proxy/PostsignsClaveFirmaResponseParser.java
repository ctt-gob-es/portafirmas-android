...
public class PostsignsClaveFirmaResponseParser {

	private static final String POSTSIGN_RESPONSE_NODE = "posts"; //$NON-NLS-1$

	private PostsignsClaveFirmaResponseParser() {
		// No instanciable
	}
	...
	static RequestResult[] parse(final Document doc) {
		...
		for (int i = 0; i < requestNodes.getLength(); i++) {
			...
			requestResults.add(RequestResultParser.parse(requestNodes.item(i)));
		}

		return requestResults.toArray(new RequestResult[requestResults.size()]);
