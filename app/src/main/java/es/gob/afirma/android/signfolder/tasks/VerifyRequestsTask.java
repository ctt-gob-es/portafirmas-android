    ...
    protected void onPostExecute(final RequestResult[] verifiedRequests) {
        ...
        for (final RequestResult rResult : verifiedRequests) {
            if (rResult.isStatusOk()) {
                this.listener.requestOperationFinished(OperationRequestListener.VERIFY_OPERATION, rResult);
            }
            else {
                this.listener.requestOperationFailed(OperationRequestListener.VERIFY_OPERATION, rResult, this.t);
            }
        }
    }
}
